...
def Preparing_for_coding(s):
    all_text = s.encode('ascii') # покамест полагаем, что все пароли на английском языке
    all_text = list(all_text)
    blocks_for_coding, i = [], 16
    while i < len(all_text):
        blocks_for_coding.append(all_text[i-16:i])
        i += 16
    last_block = all_text[i-16:]
    while len(last_block) != 16:
        last_block.append(0)
    blocks_for_coding.append(last_block)
    return blocks_for_coding

def Code(info, key):
    blocks = Preparing_for_coding(info)
    result = []
    keys = Keys_for_encryption(key)
    for x in blocks:
        result.extend(Coding(x, keys))
    return bytes(result)

## print(Code('FuckYouIloveTelegram', [i*2 for i in range(32)]))
